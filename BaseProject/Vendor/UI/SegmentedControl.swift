...
open class SegmentedControl: UIView {
    ...
    */
    ...
    /// Common initializer.
     func commonInit(_ data: Any, highlightStyle: XMSelectedItemHighlightStyle) {
    ...
    open func update() {
        func addSegments(startingPosition starting: CGFloat, sections: Int, width: CGFloat, height: CGFloat) {
            ...
            for i in 0 ..< sections {
                ...
                tab.tag = i
                tab.addTarget(self, action: #selector(SegmentedControl.segmentPressed(_:)), for: UIControlEvents.touchUpInside)
                self.addSubview(tab)
            }
        }
    ...
    @objc  func segmentPressed(_ sender: UIButton) {
