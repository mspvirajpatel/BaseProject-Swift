...
class AppBaseLayout: NSObject {
    ...
    func expandView(_ containerView: UIView, insideView mainView: UIView, topSpace spaceTop:CGFloat ){
        
        
        let dictionary: Dictionary = ["containerView" : containerView]
        
        let metrics : Dictionary = ["space" : spaceTop]
        self.control_H = NSLayoutConstraint.constraints(withVisualFormat: "H:|[containerView]|", options:NSLayoutFormatOptions(rawValue: 0), metrics: metrics, views: dictionary)
        self.control_V = NSLayoutConstraint.constraints(withVisualFormat: "V:|-space-[containerView]|", options:NSLayoutFormatOptions(rawValue: 0), metrics: metrics, views: dictionary)
