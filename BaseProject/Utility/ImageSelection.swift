...
typealias ImageSelectionComplitionBlock = (_ imageSelect:ImageSelection, _ selectedImage : UIImage?)->()

class ImageSelection: NSObject {
    
    static let manager = ImageSelection()
    
    var confirmBlock : ImageSelectionComplitionBlock?
    fileprivate var openInViewController : UIViewController!
    
    //main function used to show imageSelection options
    func openImagePicker(from viewController:UIViewController, allowEditing : Bool, confirm:@escaping ImageSelectionComplitionBlock)  {
        confirmBlock = nil
